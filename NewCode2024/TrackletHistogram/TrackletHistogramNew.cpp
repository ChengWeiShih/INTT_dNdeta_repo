...
{   
    ...
    for (int Mbin = 0; Mbin < nCentrality_bin; Mbin++){
        ...
        isInserted = h2D_map.insert( std::make_pair(
                ...
                new TH2D(Form("h2D_typeA_GoodProtoTracklet_EtaVtxZ_Mbin%d_FineBin", Mbin), Form("h2D_typeA_GoodProtoTracklet_EtaVtxZ_Mbin%d_FineBin;Pair #eta (type A);INTT vtxZ [cm]", Mbin), 400, EtaEdge_min, EtaEdge_max, 400, VtxZEdge_min, VtxZEdge_max) 
            )
        ).second; insert_check.push_back(isInserted);
    ...
    for (int Mbin = 0; Mbin < nCentrality_bin; Mbin++){
        ...
        isInserted = h2D_map.insert( std::make_pair(
                ...
                new TH2D(Form("h2D_BestPairEtaVtxZ_Mbin%d_FineBin",Mbin), Form("h2D_BestPairEtaVtxZ_Mbin%d_FineBin;Tracklet #eta;INTT vtxZ [cm]",Mbin), 400, EtaEdge_min, EtaEdge_max, 400, VtxZEdge_min, VtxZEdge_max)
            )
        ).second; insert_check.push_back(isInserted);
        ...
        isInserted = h2D_map.insert( std::make_pair(
                ...
                new TH2D(Form("h2D_typeA_BestPairEtaVtxZ_Mbin%d_FineBin",Mbin), Form("h2D_typeA_BestPairEtaVtxZ_Mbin%d_FineBin;Tracklet #eta (type A);INTT vtxZ [cm]",Mbin), 400, EtaEdge_min, EtaEdge_max, 400, VtxZEdge_min, VtxZEdge_max)
            )
        ).second; insert_check.push_back(isInserted);
    }
...
void TrackletHistogramNew::GetTrackletPair(std::vector<pair_str> &input_TrackletPair_vec, bool isRotated)
{
    ...
    std::vector<ClusHistogram::clu_info> temp_evt_sPH_inner_nocolumn_vec = (isRotated) ? GetRoatedClusterVec(evt_sPH_inner_nocolumn_vec) : evt_sPH_inner_nocolumn_vec;
        ...
        for (int inner_phi_clu_i = 0; inner_phi_clu_i < inner_clu_phi_map[inner_phi_i].size(); inner_phi_clu_i++)
        {
            ...
            for (int scan_i = -5; scan_i < 6; scan_i++)
            {
                ...
                for (int outer_phi_clu_i = 0; outer_phi_clu_i < outer_clu_phi_map[true_scan_i].size(); outer_phi_clu_i++)
                {
                    ...
                    double inner_clu_eta = get_clu_eta({vertexXYIncm.first, vertexXYIncm.second, INTTvtxZ},{inner_clu_phi_map[inner_phi_i][inner_phi_clu_i].second.x, inner_clu_phi_map[inner_phi_i][inner_phi_clu_i].second.y, inner_clu_phi_map[inner_phi_i][inner_phi_clu_i].second.z});
                    double outer_clu_eta = get_clu_eta({vertexXYIncm.first, vertexXYIncm.second, INTTvtxZ},{outer_clu_phi_map[true_scan_i][outer_phi_clu_i].second.x, outer_clu_phi_map[true_scan_i][outer_phi_clu_i].second.y, outer_clu_phi_map[true_scan_i][outer_phi_clu_i].second.z});
    ...
    for (int pair_i = 0; pair_i < input_TrackletPair_vec.size(); pair_i++) // note : either non-rotated or rotated
    {
        ...
        if (isRotated == false){
            ...
            if (this_pair.delta_phi > cut_GoodProtoTracklet_DeltaPhi.first && this_pair.delta_phi < cut_GoodProtoTracklet_DeltaPhi.second){
                h2D_map[Form("h2D_GoodProtoTracklet_EtaVtxZ_Mbin%d_FineBin", Mbin_in)] -> Fill(this_pair.pair_eta_fit, INTTvtxZ, vtxZ_weight_in);
            }
        }
        ...
        if (this_pair.delta_phi > cut_GoodProtoTracklet_DeltaPhi.first && this_pair.delta_phi < cut_GoodProtoTracklet_DeltaPhi.second)
        {
            h2D_map[Form("h2D_GoodProtoTracklet_EtaVtxZ_Mbin%d%s", Mbin_in, rotated_text.c_str())] -> Fill(this_pair.pair_eta_fit, INTTvtxZ, vtxZ_weight_in);
        }
        // note : for both rotated and non-rotated
        ...
        if (this_pair.delta_phi > cut_GoodProtoTracklet_DeltaPhi.first && this_pair.delta_phi < cut_GoodProtoTracklet_DeltaPhi.second)
        {
            h2D_map[Form("h2D_typeA_GoodProtoTracklet_EtaVtxZ_Mbin%d%s", Mbin_in, rotated_text.c_str())] -> Fill(this_pair.pair_eta_fit, INTTvtxZ, vtxZ_weight_in);
        }
        // note : for both rotated and non-rotated

        if (isRotated == false){
            if (this_pair.delta_phi > cut_GoodProtoTracklet_DeltaPhi.first && this_pair.delta_phi < cut_GoodProtoTracklet_DeltaPhi.second){
                h2D_map[Form("h2D_typeA_GoodProtoTracklet_EtaVtxZ_Mbin%d_FineBin", Mbin_in)] -> Fill(this_pair.pair_eta_fit, INTTvtxZ, vtxZ_weight_in);
            }
        }
    }
...
std::vector<ClusHistogram::clu_info> TrackletHistogramNew::GetRoatedClusterVec(std::vector<ClusHistogram::clu_info> input_cluster_vec)
{
