...
class INTTZvtx
{
    ...
    private : 
        ...
        TH2F * Z_resolution_Nclu;     // note : N good hits / width
        TH2F * Z_resolution_pos;      // note : N good hits / width
        TH2F * Z_resolution_pos_cut;  // note : N good hits / width
        TH1F * Z_resolution;          // note : N good hits / width
        TH1F * evt_possible_z;
        ...
        vector<vector<double>> good_track_xy_vec;
        vector<vector<double>> good_track_rz_vec;
        vector<vector<double>> good_comb_rz_vec;
        vector<vector<double>> good_comb_xy_vec;
        vector<vector<double>> good_comb_phi_vec;
        vector<vector<double>> good_tracklet_rz;
        vector<float> temp_event_zvtx_vec;
...
void INTTZvtx::InitHist()
{
    ...
    zvtx_evt_width = new TH1F("","zvtx_evt_width",150,0,800);
    ...
    zvtx_evt_nclu_corre = new TH2F("","zvtx_evt_nclu_corre",200,0,10000,200,-500,500);
    ...
    width_density = new TH1F("","width_density",200,0,40); // note : N good hits / width
    ...
    Z_resolution_Nclu = new TH2F("","Z_resolution_Nclu",200,0,10000,200,-100,100); // note : N good hits / width
    ...
    Z_resolution_pos = new TH2F("","Z_resolution_pos",200,-350,350,200,-100,100); // note : N good hits / width
    ...
    Z_resolution_pos_cut = new TH2F("","Z_resolution_pos_cut",200,-350,350,200,-100,100); // note : N good hits / width
    ...
    Z_resolution = new TH1F("","Z_resolution",200,-100,100); // note : N good hits / width
    ...
    evt_possible_z = new TH1F("","evt_possible_z",100,-700,700);
    ...
    gaus_rchi2_Nclu = new TH2F("","gaus_rchi2_Nclu",200,0,10000,200,0,25);
...
{
    c2 = new TCanvas("","",3400,800);    
    c2 -> cd();
    pad_xy = new TPad(Form("pad_xy"), "", 0.0, 0.0, 0.25, 1.0);
    Characterize_Pad(pad_xy, 0.15, 0.1, 0.1, 0.1 , 0, 0);
    pad_xy -> Draw();

    pad_rz = new TPad(Form("pad_rz"), "", 0.25, 0.0, 0.50, 1.0);
    Characterize_Pad(pad_rz, 0.15, 0.1, 0.1, 0.1 , 0, 0);
    pad_rz -> Draw();

    pad_z = new TPad(Form("pad_z"), "", 0.50, 0.0, 0.75, 1.0);
    Characterize_Pad(pad_z, 0.15, 0.1, 0.1, 0.1 , 0, 0);
    pad_z -> Draw();
    
    pad_z_hist = new TPad(Form("pad_z_hist"), "", 0.75, 0.0, 1, 1.0);
...
{
    gaus_fit = new TF1("gaus_fit",gaus_func,-600,600,4);
...
)
{
    ...
    if (zvtx_cal_require < N_comb.size() && draw_event_display == true)
    {   
